...
const formState = reactive({
  p12: localStorage.getItem('p12pushcertificate') ? [localStorage.getItem('p12pushcertificate')] : [],
  password: '',//密码
  topic: '',//证书名称
...
const { resetFields, validate, validateInfos, mergeValidateInfo } = useForm(formState, rulesRef);
...
async function doUpload({ file }) {
  ...
  const res = await requestImage.post(`${API_P12_URL}/api/file/mdm/p12/upload`, formData);
...
const getIsUploadP12 = computed(() => {
  return (localStorage.getItem('p12pushcertificate') || p12Status.value == 'true') ? '已上传' : ''
})
