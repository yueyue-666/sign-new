  ...
  <div class="ele-body">
    <a-card :bordered="false">
      ...
      >
        <a-row :gutter="8">
          ...
          <a-col :xl="5" :lg="6" :md="7" :sm="24" :xs="24" v-else>
            <a-form-item label="应用类型">
              <a-select
                v-model:value="form.orderParamro"
                placeholder="请选择"
                allow-clear
              >
                <a-select-option value="0">最近更新</a-select-option>
      ...
      <a-row :gutter="8" style="padding: 0 0 10px 0">
        <a-col :xl="5" :lg="6" :md="7" :sm="24" :xs="24">
          <a-select
            v-model:value="signform.cerNameOld"
            placeholder="请选择"
            allow-clear
          >
            <a-select-option
              :value="item"
              v-for="(item, i) in SignNewList"
              :key="i"
              >{{ item }}</a-select-option
            > </a-select
          >&nbsp;替换为&nbsp;
          <a-select
            v-model:value="signform.cerNameNew"
            placeholder="请选择"
            allow-clear
          >
            <a-select-option
      ...
      >
        ...
        <template #bodyCell="{ column, record }">
            ...
            >
              ...
              <div class="nameright">
                <span>
                  ...
                  <span v-if="record.signType == 2" class="namestyle"
                    >混合</span
                  >
                </span>
                <span>
            ...
            <a-space>
              <template v-if="userRole === '3'">
                ...
                <a-popconfirm
                  placement="topRight"
                  title="确认立即V3签名吗？"
                  @confirm="signV3(record)"
                >
                  <a class="ele-text-success">签名V3</a>
                </a-popconfirm>
                <a-divider type="vertical" />
                <a class="ele-text-success" @click="setkouliang(record)"
                  >扣量比例</a
                >
                <a-divider type="vertical" />
              </template>
          ...
          <template v-else-if="column.key === 'urlType'">
            <a-select
              v-model:value="record.urlType"
              placeholder="请选择"
              allow-clear
              @change="pick(record, record.urlType)"
            >
            ...
            >
              <p>
                <span>
                  主用：
                  <a
                    :href="record.iosDownloadUrl"
                    target="_blank"
                    rel="noopener noreferrer"
                    >{{ record.iosDownloadUrl }}</a
                  >
                  <copy-outlined
                    @click="copyDetail(record.iosDownloadUrl)"
              ...
              <p>
                <span>
                  备用：
                  <a
                    :href="record.backupUrl"
                    target="_blank"
                    rel="noopener noreferrer"
                    >{{ record.backupUrl }}</a
                  >
                  <copy-outlined
                    @click="copyDetail(record.backupUrl)"
                    style="font-size: 15px"
                  />
                </span>
              </p>
    ...
    >
      <a-form
        ref="setkouliangformRef"
        :model="setkouliangform"
        :label-col="{ flex: '70px' }"
        :wrapper-col="{ flex: '1' }"
      >
        <a-form-item>
          <a-input
            class="ele-fluid"
            placeholder="请输入"
            v-model:value="setkouliangform.scale"
          />
        </a-form-item>
      </a-form>
    </ele-modal>
