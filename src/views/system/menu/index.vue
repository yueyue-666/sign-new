      ...
      >
        <template #toolbar>
          <a-space>
            ...
            <a-button type="dashed" class="ele-btn-icon" @click="expandAll">展开全部</a-button>
            <a-button type="dashed" class="ele-btn-icon" @click="foldAll">折叠全部</a-button>
          </a-space>
        </template>
        <template #bodyCell="{ column, record }">
          <template v-if="column.key === 'menuType'">
            <a-tag v-if="isExternalLink(record.path)" color="red">外链</a-tag>
            <a-tag v-else-if="isExternalLink(record.component)" color="orange">内链</a-tag>
